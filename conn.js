...
module.exports = async (conn, msg, m, setting, store) => {
  try {
    let { ownerNumber, botName, smm_dana_nama, smm_dana_number, AIapi } = setting
  ...
  upload: formatSize(out)
    ...
    if (isGroup && isAutoDownTT) {
      if (chats.match(/(tiktok.com)/gi)) {
        ...
        await sleep(1000)
        try {
          var ttdl_auto = await fetchJson(`https://saipulanuar.ga/api/download/tiktok?url=${chats}`)
          if (ttdl_auto.result.username == "") return reply(`Url timdak valid cok.`)
...
let rez = await fetchJson(`https://web-production-67cf.up.railway.app/api/yt/playmp4?query=${q}&apikey=APIKEY`)
var txt_play = `*YOUTUBE - PLAY*

*Title:* ${rez.title}
*Channel:* ${rez.channel}
*Published:* ${rez.published}
*Views:* ${rez.views}
`
var btn_ply = [
...
conn.sendMessage(from, { caption: txt_play, image: { url: rez.thumb }, buttons: btn_ply, footer: '© Gurabot - MD' })
break
      case 'playmp3':
        if (cekUser("id", sender) == null) return reply(mess.OnlyUser)
        try {
          if (!q) return reply('*Contoh:*\n#playmp3 story wa angel baby 30 detik')
          let z = await fetchJson(`https://web-production-67cf.up.railway.app/api/yt/playmp3?query=${q}&apikey=APIKEY`)
          conn.sendMessage(from, { audio: { url: z.url }, mimetype: 'audio/mpeg', fileName: z.title + 'mp3' }, { quoted: msg })
        } catch (err) {
          reply(err)
        }
        break
  ...
  itu artinya url tidak ditemukan.`)
      ...
      case 'playmp4':
        ...
        try {
          ...
          let play_m = await fetchJson(`https://web-production-67cf.up.railway.app/api/yt/playmp4?query=${q}&apikey=APIKEY`)
      ...
      case 'ytmp3': case 'mp3':
        ...
        try {
          ...
          let mmny = await fetchJson(`https://web-production-67cf.up.railway.app/api/download/ytmp3?url=${q}&apikey=APIKEY`)
      ...
      case 'ytmp4': case 'mp4':
        ...
        let mmm = await fetchJson(`https://web-production-67cf.up.railway.app/api/download/ytmp4?url=${q}&apikey=APIKEY`)
        if (mmm.status == false) return reply('Url yang anda masukan tidak valid!')
        console.log(mmm.result)
        conn.sendMessage(from, { video: { url: mmm.result.url }, caption: 'Done...' }, { quoted: msg })
        break
      case 'mediafire':
        if (cekUser("id", sender) == null) return reply(mess.OnlyUser)
...
jumlah spam: ${jumlah_pesan}`)
      ...
      case 'yuri': {
        ...
        conn.sendMessage(m.chat, { image: { url: random }, caption: `Nih Kak` }, { quoted: msg })
      }
        break
      case 'jadibot': {
        if (cekUser("id", sender) == null) return reply(mess.OnlyUser)
...
- *Tayang di* : ${i.playingAt}\n❉─────────────────────❉`
      ...
      case 'bug5': {
        ...
        case 'tiktokdl': case 'ttdl': case 'tiktok': {
          ...
          reply(mess.wait)
          try {
          var ttdl_res = await fetchJson(`https://saipulanuar.ga/api/download/tiktok?url=${q}`)
          if (ttdl_res.result.username == "") return reply(`Url timdak valid cok.`)
...
break
          case 'ttmp3':
            ...
            break
        case 'ig': 
        if (args[0]) return reply('Opsi pilihan downloader Instagram:\n\n1) .igfoto <link>\n2) .igvideo <link>\n3) .igreel <link>\n4) .igtv <link>\n5) .igstory <link>')
        break
  ...
  reply(`Story timdack ditemukan.`)
   ...
   // if (!q.includes('facebook.com')) return reply(`Itu bukan link facebook!`)
    var fbd = await xfar.downloader.facebook(q)
      console.log(fbd)
    if (fbd.status !== 200) return reply(`Link timdack valid.`)
        if (fbd.hd.includes('mp4')){
    conn.sendMessage(from, { video: { url: fbd.hd }, caption: '*Done !*'}, { quoted: msg})
    } else {
    conn.sendMessage(from, { video: { url: fbd.sd }, caption: '*Done !*'}, { quoted: msg})
    }
    break
    ...
    break
      case 'ytdl':
        ...
        reply(mess.wait)
        var yytt = await fetchJson(`https://web-production-67cf.up.railway.app/api/download/ytmp3?url=${q}&apikey=APIKEY`)
